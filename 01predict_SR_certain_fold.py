...
def predict(rank, gen_num_events, fold, args):

    device = torch.device("cuda:0")  # because SLURM bound you already
    ...
    print(f"[Rank {rank}] Starting prediction on {device}")
...
def main():
    ...
    predict(args.rank, int(args.gen_num_events / world_size), args.fold, args)
