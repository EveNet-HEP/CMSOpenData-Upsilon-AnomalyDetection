...
bin_percent = {12: 1.5} #{8: 2.3, 12: 1.5, 16: 1.1}
...
def plot_histograms_with_fits(
    ...
    save_data["SB_left"] = SB_left
    save_data["SR_left"] = SR_left
    save_data["SR_right"] = SR_right
    save_data["SB_right"] = SB_right
    save_data["popts"] = []
    save_data["pcovs"] = []
    ...
    save_data["plot_bins_all"] = plot_bins_all
    save_data["plot_bins_SR"] = plot_bins_SR
    save_data["plot_bins_left"] = plot_bins_left
    save_data["plot_bins_right"] = plot_bins_right
    save_data["plot_centers_all"] = plot_centers_all
    save_data["plot_centers_SR"] = plot_centers_SR
    save_data["plot_centers_SB"] = plot_centers_SB
    ...
    def process_threshold(t, threshold):
        ...
        label_string = f"{round(100 * threshold, 2)}%, {round(np.sqrt(q0), 2)}$\sigma$"
    ...
    plot_fpr = [1.0, 0.3, 0.1, 0.01, 0.001]
    ...
    if fit_result is None:
        ...
        #scale_factor = np.mean(weights*weights)/np.mean(weights) #TODO: for plotting, but likelihood take care of the weighting more correctly"
        scale_factor = 1.0
        color_weighted = "#D55E00"
        plt.plot(plot_centers_all, parametric_fit(plot_centers_all, *popt)/scale_factor, lw=2, linestyle="dashed", color=color_weighted)
        function_with_band(ax, parametric_fit, [SB_left, SB_right], popt, pcov, color=color_weighted, scale_factor=scale_factor)
        plt.hist(
            filtered_masses, bins=plot_bins_all, lw=10, histtype="step", color=color_weighted,
            label="l-reweighted", alpha=0.75, weights=weights/scale_factor)
        plt.scatter(plot_centers_SB, y_vals/scale_factor, color=color_weighted)

        save_data["popts"].append(popt)
        save_data["pcovs"].append(pcov)
        save_data["filtered_masses"].append(filtered_masses)
        save_data["y_vals"].append(y_vals)
        save_data["l-weighting"] = weights
        ...
        save_data["significances"].append(bonus_q0)
        save_data["fpr_thresholds"].append(-1)
...
def bump_hunting(args, num_SR_bins=8, fit_degree=3, seed=42):
    postfix = "" if not args.no_signal else "_no_signal"
    postfix += "_calibrated" if args.calibrated else ""
    ...
    postfix_for_plot = postfix
    if args.evenet:
        postfix_for_plot += "_evenet"
    plotdir = os.path.join(clean_and_append(config["output"]["plotdir"], postfix_for_plot), step_dir)
    os.makedirs(plotdir, exist_ok=True)


    # Get the data for the region
    fpr_thresholds = [1.0, 0.3, 0.1, 0.01]
    ...
    score_str = "_score" if not args.evenet else "_score_evenet"
    fit_str = "_fit" if not args.evenet else "_fit_evenet"

    if not args.test_no_signal:
        fname = os.path.join(clean_and_append(inputdir, score_str), f"final_results-seed{seed}.csv")
        os.makedirs(clean_and_append(inputdir, fit_str), exist_ok=True)
        outname = os.path.join(clean_and_append(inputdir, fit_str), f"bump_hunting_{num_SR_bins}bins_{fit_type[fit_degree]}_fit_seed{seed}.json")
        out_plotdir = os.path.join(clean_and_append(inputdir, fit_str), "plots")
        os.makedirs(out_plotdir, exist_ok=True)
        ...
        save_data, bonus_sigificance = plot_histograms_with_fits(
            ...
            channel = channel,
            plotdir = out_plotdir,
            score_name = "xgb_prob" if not args.evenet else "evenet_prob"
        )
        plt.savefig(os.path.join(out_plotdir, f'bump_hunting_{num_SR_bins}bins_{fit_type[fit_degree]}_seed{seed}_fit_xgb.png'))
        plt.savefig(os.path.join(out_plotdir, f'bump_hunting_{num_SR_bins}bins_{fit_type[fit_degree]}_seed{seed}_fit_xgb.pdf'))
        ...
        with open(outname, "w") as f:
            json.dump({"q": bonus_sigificance}, f)
        ...
        np.savez_compressed(
            outname.replace(".json", ".npz"),
            payload = np.array(payload, dtype=object)
        )

    else:
        fname = os.path.join(clean_and_append(inputdir, score_str), f"final_results_no_signal-seed{seed}.csv")
        os.makedirs(clean_and_append(inputdir, fit_str), exist_ok=True)
        outname = os.path.join(clean_and_append(inputdir, fit_str), f"bump_hunting_{num_SR_bins}bins_{fit_type[fit_degree]}_fit_no_signal_seed{seed}.json")
        ...
        plt.savefig(os.path.join(plotdir, f'bump_hunting_{num_SR_bins}bins_{fit_type[fit_degree]}_fit_xgb_nosignal.pdf'))
...
def run_bump_hunting(params):
    args, num_SR_bins, fit_degree, seed = params
    ...
    return num_SR_bins, fit_degree, seed, fpr_thresholds, save_data, bonus_significance
...
def main():
    ...
    parser.add_argument("--evenet", action = "store_true", default = False,)
    ...
    if not args.test_no_signal:
        files = [f for f in os.listdir(clean_and_append(inputdir, score_str)) if f.startswith("final_results-seed") and f.endswith(".csv")]
    else:
        files = [f for f in os.listdir(clean_and_append(inputdir, score_str)) if f.startswith("final_results_no_signal-seed") and f.endswith(".csv")]
    ...
    if args.seed is not None:
        ...
        for f, seed in zip(files, seeds):
            if int(seed) in args.seed:
                final_files.append(f)
                print(f"Found file: {f} with seed: {seed}")
                final_files.append(f)
        files = final_files
        seeds = args.seed

    # TODO: Limit to 128 seeds for now
    # seeds = seeds[:500]

    # Create all combinations of parameters to evaluate
    param_grid = [(args, num_SR_bins, fit_degree, seed) for num_SR_bins in bin_percent for fit_degree in fit_type for seed in seeds]
    ...
    for num_SR_bins, fit_degree, seed, fpr_thresholds, save_data, bonus_significance in results:
        if num_SR_bins not in summary_data:
            summary_data[num_SR_bins] = dict()
        if fit_degree not in summary_data[num_SR_bins]:
            summary_data[num_SR_bins][fit_degree] = dict()
        summary_data[num_SR_bins][fit_degree][seed] = {
            # "fpr_thresholds": fpr_thresholds,
            # "save_data": save_data,
