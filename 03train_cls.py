...
def train(args):
    ...
    print(f"✅ Loaded data with {df.shape[0]} events (SB: {df_SB.shape[0]}, SR: {df.shape[0]}) from {inputdir}")

    test_no_signal = args.test_no_signal
    if test_no_signal:
        if args.no_signal:
            test_postfix = "" if  test_no_signal else "_no_signal"
        else:
            test_postfix = "_no_signal" if test_no_signal else ""
        test_postfix += "_calibrated" if args.calibrated else ""
        input_no_signal = clean_and_append(clean_and_append(config["output"]["storedir"], "_hybrid_raw"), test_postfix)
        ...
        print(f"✅ Loaded no signal data with {df_no_signal.shape[0]} events (SB: {df_SB_no_signal.shape[0]}, SR: {df_SR_no_signal.shape[0]}) from {input_no_signal}")

    if args.n_gensample is not None:
        df_background = df[df["classification"]==0]
        if args.n_gensample == -1:
            pick_number = df[df["classification"]==1].shape[0]
        else:
            pick_number = min(args.n_gensample, df_background.shape[0])
        df_background = df_background.sample(n=pick_number, random_state=args.seed)
        df_signal = df[df["classification"]==1]
        df = pd.concat([df_signal, df_background], ignore_index=True)
        df = df.sample(frac=1, random_state=args.seed).reset_index(drop=True)  # Shuffle the dataset
    ...
    for ifold, ((_, test_idx_SB), (train_idx, test_idx)) in enumerate(zip((kf_SB.split(df_SB)),(kf.split(X_scaled, y)))):
        ...
        for i_tree in range(args.n_ensemble):
            ...
            feature_names_xgb = X_train.columns if hasattr(X_train, "columns") else [f"f{i}" for i in range(X_train.shape[1])]
...
def main():
    ...
    parser.add_argument("--n_gensample", type=int, default=None, help="Number of generated samples")
