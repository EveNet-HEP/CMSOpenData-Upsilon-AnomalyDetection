...
def function_with_band(ax, f, range, params, pcov = None, color = "purple", alpha_line = 0.75, alpha_band = 0.25, lw = 3, scale_factor=1., **kwargs):
    ...
    if pcov is not None:
        ...
        y_mean = y_mean / scale_factor
        y_std = y_std/scale_factor
...
def read_feature(x, event_info, feature):
    ...
    if "log" in normal.lower():
        ...
        input_x = np.maximum(input_x, 0)
    return input_x


def write_feature(x, event_info, feature, y):
    """
    Writes feature y into x, applying the inverse of the normalization if needed.
    """
    index = list(event_info['INPUTS']['SEQUENTIAL']['Source']).index(feature)
    normal = event_info['INPUTS']['SEQUENTIAL']['Source'][feature]

    # Apply inverse normalization if log
    if "log" in normal.lower():
        y = np.log1p(np.maximum(y, 0))  # match the read_feature inverse
    # Update x
    x[..., index] = y
    return x
